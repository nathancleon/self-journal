  ...
  render() {
    ...
    return (
      <SelectedPromptContainer>
        <SelectedPromptData>
          <h1>Journal from {moment(this.props.journal.created).format("LL")}</h1>
          <div>
            <h4>{promptData.questions.self.question}</h4>
            <p>Answer: {convertAnswerNumber(this.props.journal.answerSelf, promptData.response.self.answers)}</p>
            <p>Answer Text: {this.props.journal.answerTextSelf}</p>
          </div>
          <div>
            <h4>{promptData.questions.anxiety.question}</h4>
            <p>Answer: {convertAnswerNumber(this.props.journal.answerAnxiety, promptData.response.anxiety.answers)}</p>
            <p>Answer Text: {this.props.journal.answerTextAnxiety}</p>
          </div>
          <div>
            <h4>{promptData.questions.depression.question}</h4>
            <p>Answer: {convertAnswerNumber(this.props.journal.answerDepression, promptData.response.depression.answers)}</p>
            <p>Answer Text: {this.props.journal.answerTextDepression}</p>
          </div>
          <div>
            <h4>{promptData.questions.concentration.question}</h4>
            <p>Answer: {convertAnswerNumber(this.props.journal.answerConcentration, promptData.response.concentration.answers)}</p>
            <p>Answer Text: {this.props.journal.answerTextConcentration}</p>
          </div>
          <div>
            <h4>{promptData.questions.family.question}</h4>
            <p>Answer: {convertAnswerNumber(this.props.journal.answerFamily, promptData.response.family.answers)}</p>
            <p>Answer Text: {this.props.journal.answerTextFamily}</p>
          </div>
          <div>
            <h4>{promptData.questions.friendships.question}</h4>
            <p>Answer: {convertAnswerNumber(this.props.journal.answerFriendships, promptData.response.friendships.answers)}</p>
